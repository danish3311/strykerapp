...
public class RsV2 extends AsyncTask<Void, String, Router> {
    ...
    protected Router doInBackground(Void... command) {
        ...
        AuthLists.ensureDeployed(mActivity);

        RouterScanLog.info(mContext, ip, "spawn rs " + ip);
        try {
            ...
            while ((line = br.readLine()) != null) {
                out2.add(line);
                RouterScanLog.append(mContext, ip, "OUT", line);
            ...
            proc.destroy();
            r = rs_result(out2);
            RouterScanLog.info(mContext, ip, r.getSuccess() ? "cracked" : "no match");
